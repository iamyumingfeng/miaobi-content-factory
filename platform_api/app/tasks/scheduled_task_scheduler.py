...
if _sync_db_url.startswith("mysql+aiomysql"):
    ...
    logger.info(f"[ScheduledTaskScheduler] 数据库驱动已转换: aiomysql → pymysql")
...
def check_and_execute_scheduled_tasks(self):
    ...
    try:
        ...
        logger.info(
            f"[ScheduledTaskScheduler] 发现 {len(pending_tasks)} 个待执行任务"
        )
        ...
        for task in pending_tasks:
            try:
                ...
                success, generation_task_id, error = ScheduledTaskService.execute_scheduled_task_sync(
                    db, task.id
                )
        ...
        logger.error(
            f"[ScheduledTaskScheduler] 扫描异常 | error={str(e)}",
            exc_info=True
        )
        # 重试整个扫描任务
        ...
        if db:
            try:
                ...
                logger.error(f"[ScheduledTaskScheduler] 关闭数据库连接失败 | error={str(close_err)}")
        ...
        if engine:
            try:
                ...
                logger.error(f"[ScheduledTaskScheduler] 释放数据库引擎失败 | error={str(dispose_err)}")
...
ScheduledTaskService.execute_scheduled_task_sync = staticmethod(execute_scheduled_task_sync)
