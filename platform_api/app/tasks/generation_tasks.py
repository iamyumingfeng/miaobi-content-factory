...
def _dispatch_tasks_from_queue(owner_operator_id: int, max_dispatch: int = 10) -> int:
    ...
    for _ in range(max_dispatch):
        ...
        if not task_queue_manager.can_dispatch():
            _task_info(
                "[PAUSED] 达到最大并发数，暂停调度 | active=%s | max=%s",
        ...
        try:
            ...
            dispatched_count += 1
            _task_info(
                "[START] 调度任务到Worker | item_id=%s | celery_id=%s",
                item_id,
                celery_task.id,
            )
...
async def _start_generation_task_async(task_id: int, owner_operator_id: int):
    ...
    async with async_session_maker() as db:
        ...
        if task.status in ("completed", "failed", "cancelled"):
            ...
            raise ValueError(
                f"任务已是终态({task.status})，无法重新启动，请使用重试功能"
            )

        _task_info(
            "▶ 任务状态 pending → processing | task_id=%s | items=%s",
            task_id,
            task.total_count,
        )
        # 更新任务状态为处理中
        ...
        for item in all_items:
            if item.status == "queued":
                _task_debug(
                    "  子任务入队 | item_id=%s | sub_user_id=%s | template_id=%s",
                    item.id,
                    item.sub_user_id,
                    item.template_id,
                )
        ...
        _task_info(
            "✔ 子任务入队完成 | task_id=%s | total=%s | queued=%s | queue_position=%s",
            task_id,
            len(all_items),
            queued_count,
            task_queue_manager.get_queue_length(),
        )
...
async def _process_generation_item_phased_async(item_id: int, owner_operator_id: int):
    ...
    """
    from app.services.generation_phases import (execute_phase2_generation,
    ...
    try:
        ...
        async with async_session_maker() as read_db:
            ...
            if item.status == "queued":
                ...
                and item.distribution_status == "pending_publish"
            ):
                can_execute = True

            if not can_execute:
                _task_info(
                    "[SKIPPED] 子任务状态不可执行 | status=%s | distribution_status=%s",
                    item.status,
        ...
        async with async_session_maker() as check_db:
            ...
            if not parent_exists:
                _task_error(
                    "[ORPHAN] 检测到孤儿子任务，直接删除 | item_id=%s | task_id=%s",
                    item_id,
                    task_id,
                )
                await GenerationService.delete_orphaned_item(
                    check_db, item_id, owner_operator_id
                )
                return {
                    "item_id": item_id,
                    "status": "deleted",
                    "reason": "orphaned_item",
                }

        # 状态更新：失败重试或已完成重新生成
        if item.status in ("failed", "completed"):
            _task_info(
                "[RESET] 重置子任务 | item_id=%s | old_status=%s", item_id, item.status
            )
        ...
        _task_info(
            "▶ [Phase/2/3] 开始AI生成 | item_id=%s | task_id=%s", item_id, task_id
        )
        ...
        if save_success:
            _task_info(
                "[OK] 三阶段处理完成 | item_id=%s | success=%s",
                item_id,
                output_data.success,
            )
            return {
        ...
        error_msg = str(e)
        _task_error(
            "[ERROR] 三阶段处理异常 | item_id=%s | error=%s", item_id, error_msg[:300]
        )
        ...
        try:
            ...
            _task_error(
                "[ERROR] 标记失败也失败 | item_id=%s | error=%s", item_id, inner_e
            )
...
@shared_task(
    bind=True,
    max_retries=3,
    name="app.tasks.generation_tasks.process_generation_item_phased",
)
def process_generation_item_phased(self, item_id: int, owner_operator_id: int):
    ...
    """
    _task_info(
        "[START] 启动三阶段生成任务 | item_id=%s | owner=%s | celery_id=%s",
        item_id,
        owner_operator_id,
        self.request.id,
    )
    ...
    try:
        ...
        # 数据孤儿问题，不应重试
        _task_error(
            "[ERROR] 不可重试错误 | item_id=%s | error=%s", item_id, str(e)[:200]
        )
        # 直接返回失败结果，不触发重试
        return {"item_id": item_id, "status": "failed", "error": str(e)}
    except Exception as e:
        _task_error(
            "[ERROR] 三阶段任务失败 | item_id=%s | error=%s", item_id, str(e)[:200]
        )
        ...
        try:
            ...
            _task_info(
                "[OK] 从活跃任务移除 | item_id=%s | active=%s",
                item_id,
                task_queue_manager.get_active_count(),
            )
        except Exception as rm_e:
            _task_error(
                "[ERROR] 移除活跃任务失败 | item_id=%s | error=%s", item_id, str(rm_e)
            )
