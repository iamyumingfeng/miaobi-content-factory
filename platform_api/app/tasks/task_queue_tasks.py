        ...
        for task_info in active_tasks:
            ...
            try:
                ...
                if task_result.ready():
                    ...
                    orphaned_count += 1
                    logger.info(
                        f"清理已完成任务记录: item_id={item_id} | status={task_result.status}"
                    )
...
def monitor_queue_health():
    ...
    try:
        ...
        if active_count >= max_concurrent:
            logger.warning(
                f"⚠️ 队列已满 | active={active_count} | max={max_concurrent} | waiting={waiting_count}"
            )
            # TODO: 发送告警通知
        ...
        logger.info(
            f"✔ 队列监控完成 | active={active_count} | waiting={waiting_count} | stale={stale_count}"
        )
