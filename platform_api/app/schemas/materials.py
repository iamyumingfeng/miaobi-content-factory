...
class MaterialTagCreate(MaterialTagBase):
    ...
    """
    category_id: Optional[int] = Field(default=None, description="所属分类ID（NULL表示未分类，属于平台层级）")
...
class MaterialBase(BaseModel):
    ...
    topic: str = Field(..., description="素材话题（必选）", min_length=1, max_length=255)
    text_content: Optional[str] = Field(default=None, description="文本内容（旧字段，保留兼容）")
    source_url: Optional[str] = Field(default=None, description="来源URL", max_length=500)
    source_type: str = Field(default="upload", description="来源类型：upload / link / description")
    content_type: str = Field(default="text", description="内容类型：text / image_text / video_text / mix")
...
class MaterialCreate(BaseModel):
    ...
    creative_seed_id: Optional[int] = Field(default=None, description="关联的创意种子ID")
...
class MaterialUpdate(BaseModel):
    ...
    更新素材请求
    """
    title: Optional[str] = Field(default=None, description="素材标题", min_length=1, max_length=500)
    content: Optional[str] = Field(default=None, description="素材内容", min_length=1)
    topic: Optional[str] = Field(default=None, description="素材话题", min_length=1, max_length=255)
...
class MaterialResponse(MaterialBase, BaseSchema):
    ...
    attachments: Optional[List[MaterialAttachmentResponse]] = Field(default=None, description="素材附件列表")
    tags: Optional[List[MaterialTagResponse]] = Field(default=None, description="素材标签列表")
...
class MaterialCopyRequest(BaseModel):
    ...
    target_operator_id: Optional[int] = Field(default=None, description="目标管理员ID（超级管理员复制给其他管理员时使用）")
    tag_ids: Optional[List[int]] = Field(default=None, description="目标标签ID列表（为空或未传则不关联标签）")
