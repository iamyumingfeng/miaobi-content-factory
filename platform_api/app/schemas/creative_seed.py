...
class CreativeSeedBase(BaseModel):
    ...
    template: str = Field(..., description="模板示例（JSON数组格式，如：['示例1', '示例2']）")
    description: Optional[str] = Field(None, description="使用说明")
    forbidden_patterns: Optional[List[str]] = Field(default_factory=list, description="禁止使用的模式")
    example_phrases: Optional[List[str]] = Field(default_factory=list, description="典型表达示例")
    avoid_phrases: Optional[List[str]] = Field(default_factory=list, description="避免的表达")
    category: Optional[str] = Field("通用", max_length=50, description="适用品类")
    status: CreativeSeedStatusEnum = Field(CreativeSeedStatusEnum.ENABLED, description="状态")
...
class CreativeSeedGroupResponse(BaseModel):
    """按类型分组的创意种子响应"""
    opening: List[CreativeSeedSelectResponse] = Field(default_factory=list, description="开头模式列表")
    emotion: List[CreativeSeedSelectResponse] = Field(default_factory=list, description="情感基调列表")
    ending: List[CreativeSeedSelectResponse] = Field(default_factory=list, description="结尾模式列表")
