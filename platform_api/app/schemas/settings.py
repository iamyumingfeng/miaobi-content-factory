...
class ModelConfigBase(BaseModel):
    ...
    模型配置基础
    """
    platform: str = Field(..., description="平台：bailian / volcano / moonshot / zhipu / jimeng / kling等", max_length=100)
    model_id: str = Field(..., description="模型ID", max_length=255)
    model_name: str = Field(..., description="模型名称", max_length=255)
    model_type: str = Field(..., description="模型类型：llm / image / video / embedding")
...
class ModelConfigUpdate(BaseModel):
    ...
    model_type: Optional[str] = Field(None, description="模型类型：llm / image / video / embedding")
    ...
    max_concurrency: Optional[int] = Field(None, description="最大并发数", ge=1, le=100000)
    config_json: Optional[dict] = Field(None, description="配置JSON（包含API Key等）")
...
class UserDefaultModelBase(BaseModel):
    ...
    """
    model_type: str = Field(..., description="模型类型：llm / image / video / embedding")
    model_config_id: Optional[int] = Field(None, description="关联的模型配置ID，NULL表示自动选择")
...
class UserDefaultModelUpdate(BaseModel):
    ...
    更新用户默认模型设置
    """
    llm_model_config_id: Optional[int] = Field(None, description="文本模型配置ID，None表示自动选择")
    image_model_config_id: Optional[int] = Field(None, description="图片模型配置ID，None表示自动选择")
    video_model_config_id: Optional[int] = Field(None, description="视频模型配置ID，None表示自动选择")
    embedding_model_config_id: Optional[int] = Field(None, description="Embedding模型配置ID，None表示自动选择")
