...
class WeeklyScheduleConfig(BaseModel):
    ...
    days: List[int] = Field(
        ..., description="执行日期列表，1=周一...7=周日，如 [1, 3, 5]", min_length=1
    )
    times: List[str] = Field(
        ...,
        description="执行时间列表，格式：HH:MM，如 ['09:00', '18:00']",
...
class ScheduledTaskBase(BaseModel):
    ...
    # 调度配置
    schedule_type: str = Field(..., description="调度类型：daily(每日) / weekly(每周)")
    ...
    material_id: Optional[int] = Field(
        default=None, description="创作库素材ID（自定义文案时使用）"
    )
    benchmark_material_ids_json: Optional[List[int]] = Field(
        default=None, description="对标素材ID列表（对标文案时使用，支持多选）"
    )
    template_ids_json: Optional[List[int]] = Field(
        default=None, description="模板ID列表（支持多选，每次执行随机选一个）"
    )
    ...
    model_id: Optional[str] = Field(
        default=None, description="选择的文本模型ID", max_length=100
    )
    ...
    image_model_id: Optional[str] = Field(
        default=None, description="选择的图片模型ID", max_length=100
    )
    model_selection_mode: str = Field(
        default="auto", description="模型选择模式：auto / manual"
    )
    max_concurrency: int = Field(
        default=5, description="本次任务最大并发数", ge=1, le=100
    )
    ...
    dedup_threshold: Optional[int] = Field(
        default=90, description="文案去重阈值（0-100）", ge=0, le=100
    )
    dedup_retry_count: Optional[int] = Field(
        default=3, description="文案去重失败重试次数", ge=0, le=10
    )
    dedup_scope: Optional[List[str]] = Field(
        default=None, description="文案去重范围配置"
    )
    ...
    image_dedup_threshold: Optional[int] = Field(
        default=90, description="图片相似度阈值（0-100）", ge=0, le=100
    )
    image_dedup_retry_count: Optional[int] = Field(
        default=3, description="图片去重失败重试次数", ge=0, le=10
    )
    image_dedup_scope: Optional[List[str]] = Field(
        default=None, description="图片去重范围"
    )
    ...
    benchmark_image_roles_json: Optional[Dict[str, List[str]]] = Field(
        default=None, description="对标图角色配置"
    )
    template_product_mapping_json: Optional[Dict[str, str]] = Field(
        default=None, description="模板产品映射"
    )
...
class ScheduledTaskUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="定时任务名称", max_length=200
    )
    task_type: Optional[str] = Field(
        default=None, description="任务类型：custom / benchmark"
    )

    # 调度配置
    schedule_type: Optional[str] = Field(
        default=None, description="调度类型：daily / weekly"
    )
    schedule_config_json: Optional[Dict[str, Any]] = Field(
        default=None, description="调度配置JSON"
    )

    # 素材和模板配置
    material_id: Optional[int] = Field(default=None, description="创作库素材ID")
    benchmark_material_ids_json: Optional[List[int]] = Field(
        default=None, description="对标素材ID列表"
    )
    template_ids_json: Optional[List[int]] = Field(
        default=None, description="模板ID列表"
    )
    sub_user_ids_json: Optional[List[int]] = Field(
        default=None, description="目标创作者ID列表"
    )
    ...
    model_selection_mode: Optional[str] = Field(
        default=None, description="模型选择模式"
    )
    max_concurrency: Optional[int] = Field(
        default=None, description="最大并发数", ge=1, le=100
    )
    image_count: Optional[int] = Field(
        default=None, description="生成图片数量", ge=1, le=10
    )
    ...
    dedup_threshold: Optional[int] = Field(
        default=None, description="文案去重阈值", ge=0, le=100
    )
    dedup_retry_count: Optional[int] = Field(
        default=None, description="文案去重失败重试次数", ge=0, le=10
    )
    dedup_scope: Optional[List[str]] = Field(
        default=None, description="文案去重范围配置"
    )
    ...
    # 图片去重配置
    image_dedup_enabled: Optional[bool] = Field(
        default=None, description="图片去重检测开关"
    )
    image_dedup_threshold: Optional[int] = Field(
        default=None, description="图片相似度阈值", ge=0, le=100
    )
    image_dedup_retry_count: Optional[int] = Field(
        default=None, description="图片去重失败重试次数", ge=0, le=10
    )
    image_dedup_scope: Optional[List[str]] = Field(
        default=None, description="图片去重范围"
    )
    ...
    status: Optional[str] = Field(
        default=None, description="任务状态：active / paused / disabled"
    )
...
class ScheduledTaskResponse(BaseSchema):
    ...
    benchmark_material_titles: Optional[List[str]] = Field(
        default=None, description="对标素材标题列表"
    )
    template_ids_json: Optional[List[int]] = Field(
        default=None, description="模板ID列表"
    )
    template_names: Optional[List[str]] = Field(
        default=None, description="模板名称列表"
    )
    sub_user_ids_json: List[int] = Field(..., description="目标创作者ID列表")
    sub_user_names: Optional[List[str]] = Field(
        default=None, description="创作者名称列表"
    )
    ...
    dedup_retry_count: Optional[int] = Field(
        default=None, description="文案去重失败重试次数"
    )
    dedup_scope: Optional[List[str]] = Field(
        default=None, description="文案去重范围配置"
    )
    ...
    image_dedup_threshold: Optional[int] = Field(
        default=None, description="图片相似度阈值"
    )
    image_dedup_retry_count: Optional[int] = Field(
        default=None, description="图片去重失败重试次数"
    )
    image_dedup_scope: Optional[List[str]] = Field(
        default=None, description="图片去重范围"
    )
    ...
    last_execution_at: Optional[datetime] = Field(
        default=None, description="最后执行时间"
    )
    last_execution_status: Optional[str] = Field(
        default=None, description="最后执行状态"
    )
    ...
    owner_operator_name: Optional[str] = Field(
...
class ScheduledTaskExecutionLog(BaseModel):
    ...
    execution_type: str = Field(
        ..., description="执行类型：scheduled(定时) / manual(手动)"
    )
    ...
    status: str = Field(
        ...,
        description="执行状态：pending(排队中) / running(执行中) / completed(完成) / failed(失败) / partial(部分成功) / cancelled(已取消)",
    )
