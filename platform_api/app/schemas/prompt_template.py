...
class PromptTemplateBase(BaseModel):
    ...
    template_type: str = Field(
        ...,
        description="提示词类型：aigc_text_prompt_generator/aigc_image_prompt_generator",
    )
    applicable_platforms: Optional[list[str]] = Field(
        default=None, description="适用平台列表"
    )
...
class PromptTemplateCopyRequest(BaseModel):
    ...
    target_operator_ids: list[int] = Field(
        ..., description="目标创作管理员ID列表", min_length=1
    )
