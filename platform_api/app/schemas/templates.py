...
class TemplatePlatformUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="平台名称", min_length=1, max_length=100
    )
    description: Optional[str] = Field(default=None, description="描述")
    color: Optional[str] = Field(default=None, description="平台颜色")
...
class TemplateTagCreate(TemplateTagBase):
    ...
    category_id: Optional[int] = Field(
        default=None, description="所属分类ID（NULL表示未分类）"
    )
...
class TemplateBase(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="创作方向说明（原提示词创意）"
    )
    content_type: str = Field(
        ..., description="内容类型：text / image_text / video_text"
    )
    prompt_template: Optional[str] = Field(default=None, description="提示词模板")
    variables_json: Optional[Dict[str, Any]] = Field(
        default=None, description="变量定义JSON"
    )
    style_reference: Optional[str] = Field(default=None, description="风格参考")
    platform_rules_json: Optional[Dict[str, Any]] = Field(
        default=None, description="平台规则JSON"
    )
    ...
    opening_seed_id: Optional[str] = Field(
        default=None, description="开头模式：'auto'表示随机，数字字符串表示指定种子ID"
    )
    emotion_seed_id: Optional[str] = Field(
        default=None, description="情感基调：'auto'表示随机，数字字符串表示指定种子ID"
    )
    ending_seed_id: Optional[str] = Field(
        default=None, description="结尾模式：'auto'表示随机，数字字符串表示指定种子ID"
    )
    # 产品名称（必填，用于提示词中明确推广的产品）
    product_name: str = Field(
        default=None,
        description="产品名称（用于提示词中明确推广的产品，防止与对标素材混淆）",
        min_length=1,
        max_length=255,
    )
    # 产品卖点
    product_selling_points: Optional[str] = Field(
        default=None, description="产品卖点描述（用于文案生成时强调重点）"
    )
...
class TemplateCreate(TemplateBase):
    ...
    # 爆款标签（可选）
    viral_tags: Optional[List[str]] = Field(
        default=None, description="爆款标签列表，如：['高互动', '高转化', '高曝光']"
    )
...
class TemplateUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        default=None, description="模板名称", min_length=1, max_length=200
    )
    ...
    # 支持：'auto' 表示随机选择，数字字符串表示指定种子ID
    opening_seed_id: Optional[str] = Field(
        default=None, description="开头模式：'auto'表示随机"
    )
    emotion_seed_id: Optional[str] = Field(
        default=None, description="情感基调：'auto'表示随机"
    )
    ending_seed_id: Optional[str] = Field(
        default=None, description="结尾模式：'auto'表示随机"
    )
    product_name: Optional[str] = Field(
        default=None, description="产品名称", min_length=1, max_length=255
    )
    product_selling_points: Optional[str] = Field(
...
class TemplateResponse(TemplateBase, BaseSchema):
    ...
    original_template_id: Optional[int] = Field(
        default=None, description="原模板ID（复制来源）"
    )
    platform: Optional[TemplatePlatformResponse] = Field(
        default=None, description="所属平台"
    )
    category: Optional[Dict[str, Any]] = Field(
        default=None, description="所属分类（从第一个标签获取）"
    )
    tags: Optional[List[TemplateTagResponse]] = Field(
        default=None, description="模板标签列表"
    )
    image_count: int = Field(default=0, description="图片数量")
    video_count: int = Field(default=0, description="视频数量")
    attachments: Optional[List[TemplateAttachmentResponse]] = Field(
        default=None, description="模板附件列表"
    )
    # ===== 爆款模板统计字段 =====
    ...
    # 创意种子详情（关联查询后填充）
    opening_seed: Optional[Dict[str, Any]] = Field(
        default=None, description="开头模式种子详情"
    )
    emotion_seed: Optional[Dict[str, Any]] = Field(
        default=None, description="情感基调种子详情"
    )
    ending_seed: Optional[Dict[str, Any]] = Field(
        default=None, description="结尾模式种子详情"
    )
...
class TemplateCopyRequest(BaseModel):
    ...
    target_tag_ids: Optional[List[int]] = Field(
        default=None, description="目标标签ID列表"
    )
