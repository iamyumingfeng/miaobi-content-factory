...
class CreativeSeed(Base):
    ...
    # 基本信息
    name = Column(
        String(100), nullable=False, comment="种子名称（如：反常识开头、轻松吐槽基调）"
    )
    seed_type = Column(
    ...
    # 禁止和推荐内容
    forbidden_patterns = Column(
        Text, nullable=True, comment="禁止使用的模式（JSON数组）"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "forbidden_patterns": (
                json.loads(self.forbidden_patterns) if self.forbidden_patterns else []
            ),
            "example_phrases": (
                json.loads(self.example_phrases) if self.example_phrases else []
            ),
            "avoid_phrases": (
                json.loads(self.avoid_phrases) if self.avoid_phrases else []
            ),
