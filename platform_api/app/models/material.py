...
from sqlalchemy import (JSON, BigInteger, Column, DateTime, Enum,
...
class MaterialPlatform(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "owner_operator_id", "name", name="uq_material_platform_owner_name"
        ),
...
class MaterialCategory(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "owner_operator_id", "name", name="uq_material_category_owner_name"
        ),
    ...
    material_platform_id = Column(
        BigInteger,
        ForeignKey("material_platform.id"),
        nullable=False,
        comment="所属素材平台ID",
    )
    owner_operator_id = Column(
    ...
    platform = relationship(
        ...
        foreign_keys=[material_platform_id],
    )
    tags = relationship(
        "MaterialTag",
...
class MaterialTag(Base):
    ...
    category = relationship(
        "MaterialCategory", back_populates="tags", foreign_keys=[category_id]
    )
...
class MaterialTagRel(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True, comment="主键")
    material_id = Column(
        BigInteger, ForeignKey("material.id"), nullable=False, comment="素材ID"
    )
    tag_id = Column(
        BigInteger, ForeignKey("material_tag.id"), nullable=False, comment="标签ID"
    )
    created_at = Column(
        DateTime, nullable=False, default=datetime.now, comment="创建时间"
    )
...
class MaterialAttachment(Base):
    ...
    material = relationship(
        "Material", back_populates="attachments", foreign_keys=[material_id]
    )
...
class Material(Base):
    ...
    content_type = Column(
        Enum(
            "text", "image_text", "video_text", "mix", name="materialcontent_type_enum"
        ),
        nullable=False,
        default="text",
    ...
    generation_tasks = relationship(
        ...
        foreign_keys="GenerationTask.material_id",
    )
