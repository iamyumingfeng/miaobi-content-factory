...
class UserBase(Base):
    ...
    userid = Column(String(64), unique=True, nullable=False, index=True, comment="用户ID（登录用）")
    nickname = Column(String(100), nullable=False, comment="【管理备注名】管理员备注名")
    display_name = Column(String(100), nullable=True, comment="【自定义昵称】用户自定义昵称")
    ...
    user_positioning = Column(String(500), nullable=True, comment="账号定位（核心赛道/价值/受众/层级等）")
    user_category = Column(String(100), nullable=True, comment="账号的运营分类")

    # 创作者专属字段（所有表统一包含，保持结构一致）
    # 要点1【粉丝画像】：年龄、职业、地域、核心需求、偏好、禁忌等
    fan_profile = Column(String(500), nullable=True, comment="粉丝画像（年龄/职业/地域/需求/偏好/禁忌等）")
    # 要点3【内容风格】：语气、排版逻辑、呈现形式、核心调性、固定句式/口头禅等
    content_style = Column(String(500), nullable=True, comment="内容风格（语气/排版/调性/句式等）")
    account_type = Column(String(255), nullable=True, comment="账号类型")

    # 时间字段
    last_login_at = Column(DateTime, nullable=True, comment="最后登录时间")
    last_password_changed_at = Column(DateTime, nullable=True, comment="最后密码修改时间")
    ...
    owner_operator_id = Column(BigInteger, nullable=True, index=True, comment="所属创作管理员ID（数据隔离用）")
    managed_by = Column(BigInteger, nullable=True, index=True, comment="当前管理者用户ID（支持转移）")
