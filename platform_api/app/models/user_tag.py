...
class UserTag(Base):
    ...
    # 关联
    user_tag_rels = relationship(
        "UserTagRel", back_populates="tag", foreign_keys="UserTagRel.tag_id"
    )
...
class UserTagRel(Base):
    ...
    tag_id = Column(
        BigInteger, ForeignKey("user_tag.id"), nullable=False, comment="标签ID"
    )
    created_at = Column(
        DateTime, nullable=False, default=datetime.now, comment="创建时间"
    )
    ...
    def __repr__(self):
        return (
            f"<UserTagRel(id={self.id}, user_id={self.user_id}, tag_id={self.tag_id})>"
        )
