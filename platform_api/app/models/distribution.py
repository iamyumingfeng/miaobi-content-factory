...
from sqlalchemy import Column, BigInteger, String, Enum, DateTime, Integer, ForeignKey
...
class Distribution(Base):
    ...
    generation_item_id = Column(BigInteger, ForeignKey("generation_item.id"), nullable=False, comment="分发的内容项")
    sub_user_id = Column(BigInteger, ForeignKey("sub_user.id"), nullable=False, index=True, comment="接收的创作者ID")
    ...
    # 关联
    task = relationship("GenerationTask", back_populates="distributions", foreign_keys=[task_id])
    generation_item = relationship("GenerationItem", back_populates="distributions", foreign_keys=[generation_item_id])
    sub_user = relationship("SubUser", back_populates="distributions", foreign_keys=[sub_user_id])
...
class PublishAccount(Base):
    ...
    # 关联
    sub_user = relationship("SubUser", back_populates="publish_accounts", foreign_keys=[sub_user_id])
