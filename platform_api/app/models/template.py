...
from sqlalchemy import Column, BigInteger, String, Enum, DateTime, Text, Integer, Float, ForeignKey, JSON, Boolean, UniqueConstraint, Index
...
class TemplateCategory(Base):
    ...
    __table_args__ = (
        UniqueConstraint('owner_operator_id', 'name', name='uq_template_category_owner_name'),
    ...
    template_platform_id = Column(BigInteger, ForeignKey("template_platform.id"), nullable=False, comment="所属模板平台ID")
    ...
    # 关联关系
    platform = relationship("TemplatePlatform", back_populates="categories", foreign_keys=[template_platform_id])
    tags = relationship(
        "TemplateTag",
...
class TemplateTag(Base):
    ...
    is_system = Column(Boolean, nullable=False, default=False, comment="是否系统默认标签")
    created_by = Column(BigInteger, ForeignKey("operator.id"), nullable=False, comment="创建者管理员ID")
    owner_operator_id = Column(BigInteger, ForeignKey("operator.id"), nullable=False, index=True, comment="所属创作管理员ID（数据隔离用）")
    created_at = Column(DateTime, nullable=False, default=datetime.now, comment="创建时间")
    updated_at = Column(DateTime, nullable=False, default=datetime.now, onupdate=datetime.now, comment="更新时间")

    # 关联关系
    category = relationship("TemplateCategory", back_populates="tags", foreign_keys=[category_id])
...
class TemplateTagRel(Base):
    ...
    tag_id = Column(BigInteger, ForeignKey("template_tag.id"), nullable=False, comment="标签ID")
    created_at = Column(DateTime, nullable=False, default=datetime.now, comment="创建时间")
...
class Template(Base):
    ...
    description = Column(String(1000), nullable=True, comment="创作方向说明（原提示词创意）")
    content_type = Column(
        Enum("text", "image_text", "video_text", name="template_content_type_enum"),
        nullable=False,
        comment="内容类型：text（纯文本）/ image_text（图文）/ video_text（视频）"
    )
    prompt_template = Column(Text, nullable=True, comment="Prompt模板（支持变量占位符）")
    ...
    platform_id = Column(BigInteger, ForeignKey("template_platform.id"), nullable=True, comment="关联模板平台ID")
    original_template_id = Column(BigInteger, ForeignKey("template.id"), nullable=True, comment="原始模板ID（用于追踪克隆关系）")
    created_by = Column(BigInteger, ForeignKey("operator.id"), nullable=True, comment="创建者创作管理员ID")
    owner_operator_id = Column(BigInteger, ForeignKey("operator.id"), nullable=False, index=True, comment="所属创作管理员ID（数据隔离用）")
    ...
    image_size_ratio = Column(String(20), nullable=True, comment="图片尺寸比例：1:1/4:3/16:9/3:4/9:16")
    # 是否添加水印
    add_watermark = Column(Boolean, nullable=False, default=True, comment="是否添加水印")
    created_at = Column(DateTime, nullable=False, default=datetime.now, comment="创建时间")
    updated_at = Column(DateTime, nullable=False, default=datetime.now, onupdate=datetime.now, comment="更新时间")
    ...
    success_count = Column(
        ...
        nullable=False,
        default=0,
        comment="成功生成次数（通过去重的数量）"
    )

    # 关联关系
    platform = relationship("TemplatePlatform", foreign_keys=[platform_id])
    generation_task_templates = relationship("GenerationTaskTemplate", back_populates="template", foreign_keys="GenerationTaskTemplate.template_id")
    generation_items = relationship("GenerationItem", back_populates="template", foreign_keys="GenerationItem.template_id")
...
class TemplateAttachment(Base):
    ...
    # 关联关系
    template = relationship("Template", back_populates="attachments", foreign_keys=[template_id])
