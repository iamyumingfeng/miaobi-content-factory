...
from sqlalchemy import Column, BigInteger, String, Enum, DateTime, Text, Integer, ForeignKey, JSON, Boolean
...
class OperationLog(Base):
    ...
    super_admin_id = Column(BigInteger, ForeignKey("super_admin.id"), nullable=True, comment="超级管理员ID")
    operator_id = Column(BigInteger, ForeignKey("operator.id"), nullable=True, comment="创作管理员ID")
    sub_user_id = Column(BigInteger, ForeignKey("sub_user.id"), nullable=True, comment="创作者ID")
    module = Column(String(50), nullable=True, comment="模块：users/templates/materials/generation/distribution/system")
    action = Column(String(100), nullable=False, comment="操作类型：create / update / delete / distribute / publish / login / logout / cancel / retry")
    description = Column(String(500), nullable=True, comment="操作描述，如：创建素材：素材标题")
    ...
    extra_data_json = Column(JSON, nullable=True, comment="额外参数（JSON格式），如标签列表、操作条件等")
    ...
    # 关联
    super_admin = relationship("SuperAdmin", back_populates="operation_logs", foreign_keys=[super_admin_id])
    operator = relationship("Operator", back_populates="operation_logs", foreign_keys=[operator_id])
    sub_user = relationship("SubUser", back_populates="operation_logs", foreign_keys=[sub_user_id])
...
class ModelConfig(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True, comment="主键")
    platform = Column(String(100), nullable=False, comment="平台：bailian / lingyaai/ zhipu / volcano / jimeng / kling 等")
    ...
    is_system = Column(Boolean, nullable=False, default=False, comment="是否系统预置模型")
    created_by = Column(BigInteger, ForeignKey("super_admin.id"), nullable=True, comment="创建者超级管理员ID")
    created_at = Column(DateTime, nullable=False, default=datetime.now, comment="创建时间")
    updated_at = Column(DateTime, nullable=False, default=datetime.now, onupdate=datetime.now, comment="更新时间")

    # 关联
    creator = relationship("SuperAdmin", back_populates="model_configs", foreign_keys=[created_by])
