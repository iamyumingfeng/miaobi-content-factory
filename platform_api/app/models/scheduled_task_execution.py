...
class ScheduledTaskExecution(Base):
    ...
    # 执行时间线
    # ============================================
    scheduled_at = Column(DateTime, nullable=False, default=datetime.now, comment="计划执行时间")
    ...
    # 执行信息（兼容旧字段）
    # ============================================
    execution_time = Column(DateTime, nullable=False, default=datetime.now, comment="执行时间（兼容字段，等价于 scheduled_at）")
    status = Column(
        String(50), 
        nullable=False,
        default="pending",
