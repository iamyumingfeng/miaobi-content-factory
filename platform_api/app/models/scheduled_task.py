...
class ScheduledTask(Base):
    ...
    # 素材和模板配置
    # ============================================
    material_id = Column(BigInteger, ForeignKey("material.id"), nullable=True, comment="创作库素材ID（自定义文案时使用）")
    benchmark_material_ids_json = Column(JSON, nullable=True, comment="对标素材ID列表JSON [1, 2, 3]（对标文案时使用，支持多选）")
    template_ids_json = Column(JSON, nullable=True, comment="模板ID列表JSON [1, 2, 3]（支持多选，每次执行随机选一个）")
    sub_user_ids_json = Column(JSON, nullable=False, comment="目标创作者ID列表JSON [1, 2, 3]")
    ...
    dedup_threshold = Column(Integer, nullable=True, default=90, comment="文案去重阈值（0-100）")
    ...
    image_dedup_threshold = Column(Integer, nullable=True, default=90, comment="图片相似度阈值（0-100）")
    ...
    last_execution_status = Column(String(50), nullable=True, comment="最后执行状态：success / failed / partial")
    
    # ============================================
    # 下次执行时间（用于调度器快速查询）
    # ============================================
    next_execution_at = Column(DateTime, nullable=True, index=True, comment="下次执行时间")
    ...
    # 关联关系
    # ============================================
    owner_operator = relationship("Operator", foreign_keys=[owner_operator_id], back_populates="scheduled_tasks")
    creator = relationship("Operator", foreign_keys=[created_by])
    material = relationship("Material", foreign_keys=[material_id])
    executions = relationship("ScheduledTaskExecution", back_populates="scheduled_task", cascade="all, delete-orphan", order_by="ScheduledTaskExecution.execution_time.desc()")
