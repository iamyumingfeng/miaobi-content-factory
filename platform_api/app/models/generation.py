...
class ContentEmbedding(Base):
    ...
    # 内容类型
    content_type = Column(
        String(20), nullable=False, index=True, comment="内容类型：text / image"
    )
    content_index = Column(
        Integer, nullable=False, default=0, comment="内容索引（多张图片时使用）"
    )
    ...
    content_preview = Column(
        String(500), nullable=True, comment="内容预览（文案前100字/图片URL）"
    )
    content_hash = Column(
        String(64),
        nullable=True,
        index=True,
        comment="内容哈希（用于快速查找重复内容）",
    )

    # 使用统计
    used_for_dedup_count = Column(
        Integer, nullable=False, default=0, comment="用于去重检测的次数"
    )
...
class GenerationTaskTemplate(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True, comment="主键")
    task_id = Column(
        BigInteger, ForeignKey("generation_task.id"), nullable=False, comment="任务ID"
    )
    template_id = Column(
    ...
    # 关联
    task = relationship(
        "GenerationTask", back_populates="task_templates", foreign_keys=[task_id]
    )
    template = relationship(
        "Template",
        back_populates="generation_task_templates",
...
class GenerationTaskSubuser(Base):
    ...
    # 关联
    task = relationship(
        "GenerationTask", back_populates="task_subusers", foreign_keys=[task_id]
    )
    sub_user = relationship(
        "SubUser", back_populates="generation_task_subusers", foreign_keys=[sub_user_id]
    )
...
class GenerationTaskProgressLog(Base):
    ...
    status = Column(
        Enum(
            ...
            "failed",
            "cancelled",
            name="generation_task_status_enum",
        ),
        nullable=False,
        comment="任务状态",
    )
    ...
    created_at = Column(
        DateTime, nullable=False, default=datetime.now, comment="记录时间"
    )

    # 关联
    task = relationship(
        "GenerationTask", back_populates="progress_logs", foreign_keys=[task_id]
    )
...
class GenerationTask(Base):
    ...
    pending_publish_count = Column(
        Integer, nullable=False, default=0, comment="待发布数量"
    )
    ...
    started_at = Column(
        DateTime,
        nullable=True,
        comment="任务开始处理时间（首次变为 processing 时设置）",
    )
    completed_at = Column(
        DateTime, nullable=True, comment="任务完成时间（变为最终状态时设置）"
    )
    ...
    image_dedup_threshold = Column(
        Float, nullable=True, default=0.9, comment="图片相似度阈值（高于此值认为相似）"
    )
    ...
    # 图片角色配置（交互式编辑）
    benchmark_image_roles_json = Column(
        JSON,
        nullable=True,
        comment="对标图角色配置，如{'image_1': ['composition', 'scene'], 'image_2': ['style']}",
    )
    template_product_mapping_json = Column(
        JSON,
        nullable=True,
        comment="模板产品映射，如{'product_1': 'template_images[0]'}",
    )

    # 关联
    owner_operator = relationship(
        "Operator", back_populates="generation_tasks", foreign_keys=[owner_operator_id]
    )
    material = relationship(
        "Material", back_populates="generation_tasks", foreign_keys=[material_id]
    )
    ...
    task_templates = relationship(
        ...
        back_populates="task",
        foreign_keys="GenerationTaskTemplate.task_id",
        cascade="all, delete-orphan",
    )
    task_subusers = relationship(
        "GenerationTaskSubuser",
        back_populates="task",
        foreign_keys="GenerationTaskSubuser.task_id",
        cascade="all, delete-orphan",
    )
    progress_logs = relationship(
        "GenerationTaskProgressLog",
        back_populates="task",
        foreign_keys="GenerationTaskProgressLog.task_id",
        cascade="all, delete-orphan",
    )
    generation_items = relationship(
        "GenerationItem",
        back_populates="task",
        foreign_keys="GenerationItem.task_id",
        cascade="all, delete-orphan",
    )
    distributions = relationship(
        "Distribution", back_populates="task", foreign_keys="Distribution.task_id"
    )
...
class GenerationItem(Base):
    ...
    generated_image_urls_json = Column(
        JSON, nullable=True, comment="生成的图片URL列表JSON"
    )
    generated_image_thumbnails_json = Column(
        JSON, nullable=True, comment="生成的图片缩略图URL列表JSON"
    )
    ...
    status = Column(
        Enum(
            ...
            "failed",
            "paused",
            name="generation_item_status_enum",
        ),
        nullable=False,
        default="queued",
        index=True,
    ...
    # 关联
    task = relationship(
        "GenerationTask", back_populates="generation_items", foreign_keys=[task_id]
    )
    sub_user = relationship(
        "SubUser", back_populates="generation_items", foreign_keys=[sub_user_id]
    )
    template = relationship(
        "Template", back_populates="generation_items", foreign_keys=[template_id]
    )
    owner_operator = relationship(
        "Operator", back_populates="generation_items", foreign_keys=[owner_operator_id]
    )
    final_prompt = Column(Text, nullable=True, comment="最终发送给模型的完整提示词")
    distributions = relationship(
        "Distribution",
        back_populates="generation_item",
        foreign_keys="Distribution.generation_item_id",
    )
    execution_logs = relationship(
        "GenerationItemExecutionLog",
        back_populates="item",
        foreign_keys="GenerationItemExecutionLog.item_id",
        cascade="all, delete-orphan",
    )
    ...
    # 模板信息
    input_prompt_creativity = Column(
        Text, nullable=True, comment="模板提示词创意（description）"
    )
    input_prompt_instruction = Column(
        Text, nullable=True, comment="模板提示词指令（prompt_template）"
    )
    input_template_images_json = Column(JSON, nullable=True, comment="模板图片URL列表")
    input_image_size_ratio = Column(
        String(20), nullable=True, comment="输出图片尺寸比例（如 16:9）"
    )
    input_watermark = Column(
        Integer, nullable=True, comment="输出图片水印开关（1=开 0=关）"
    )
    ...
    input_benchmark_images_json = Column(
        JSON, nullable=True, comment="素材对标图片URL列表"
    )
    # 素材对标配置快照
    input_benchmark_text_enabled = Column(
        Boolean, nullable=True, comment="文案对标开关"
    )
    input_benchmark_image_enabled = Column(
        Boolean, nullable=True, comment="图片对标开关"
    )
    input_benchmark_image_reference_options = Column(
        JSON, nullable=True, comment="图片参考选项"
    )
    # 图片角色配置快照（交互式编辑）
    input_benchmark_image_roles_json = Column(
        JSON, nullable=True, comment="对标图角色配置快照"
    )
    input_template_product_mapping_json = Column(
        JSON, nullable=True, comment="模板产品映射快照"
    )

    # 创作者信息
    input_sub_user_profile = Column(Text, nullable=True, comment="创作者粉丝画像")
    input_sub_user_positioning = Column(
        String(500), nullable=True, comment="创作者账号定位"
    )
    ...
    # ============================================
    aigc_generated_prompt = Column(
        Text, nullable=True, comment="AIGC提示词生成器产出的精炼提示词"
    )
    output_system_text_prompt = Column(
        Text, nullable=True, comment="AIGC文案系统提示词"
    )
    ...
    execution_result = Column(
        String(20), nullable=True, comment="执行结果：success / failed / partial"
    )

    # ============================================
    # AIGC 生成的用户提示词
    # ============================================
    aigc_user_text_generator_user_prompt = Column(
        Text, nullable=True, comment="AIGC生成的文案用户提示词"
    )
    aigc_user_image_prompts_json = Column(
        JSON, nullable=True, comment="AIGC生成的图片用户提示词列表"
    )
    ...
    dedup_referenced_items_json = Column(
        JSON, nullable=True, comment="文案相似内容引用"
    )
    ...
    # 图片去重检测结果
    # ============================================
    image_dedup_passed = Column(
        Boolean, nullable=True, comment="图片去重检测是否通过（整批图片）"
    )
    image_dedup_max_similarity = Column(Float, nullable=True, comment="图片最高相似度")
    image_dedup_similarities_json = Column(
        JSON, nullable=True, comment="每张图片的相似度列表"
    )
    image_dedup_referenced_images_json = Column(
        JSON, nullable=True, comment="相似图片引用"
    )
    image_dedup_checked_at = Column(DateTime, nullable=True, comment="图片去重检测时间")
    image_dedup_retry_count = Column(
        Integer, nullable=True, default=0, comment="图片去重实际重试次数"
    )
...
class GenerationItemExecutionLog(Base):
    ...
    node_name = Column(
        String(50),
        nullable=False,
        comment="节点名称: prompt_build / llm_call / image_call / save_result",
    )
    node_status = Column(
        Enum(
            "running", "success", "failed", "skipped", name="execution_node_status_enum"
        ),
        nullable=False,
    ...
    # 关联
    item = relationship(
        "GenerationItem", back_populates="execution_logs", foreign_keys=[item_id]
    )
