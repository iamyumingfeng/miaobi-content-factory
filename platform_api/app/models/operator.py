...
class Operator(UserBase):
    ...
    generation_tasks = relationship(
        ...
        foreign_keys="GenerationTask.owner_operator_id",
    )
    generation_items = relationship(
        "GenerationItem",
        back_populates="owner_operator",
        foreign_keys="GenerationItem.owner_operator_id",
    )
    operation_logs = relationship(
        "OperationLog",
        back_populates="operator",
        foreign_keys="OperationLog.operator_id",
    )
    scheduled_tasks = relationship(
        "ScheduledTask",
        foreign_keys="ScheduledTask.owner_operator_id",
        back_populates="owner_operator",
    )
