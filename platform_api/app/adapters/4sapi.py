...
class FourSAPIAdapter(BaseModelAdapter):
    ...
    def __init__(self, config: ModelConfig):
        ...
        logger.info(f"[4sapi] 适配器初始化 | base_url={self.base_url} | model_id={config.model_id}")
    ...
    async def generate_text(
        ...
        try:
            ...
            # 详细输入日志
            logger.info("[LLM] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            logger.info("[LLM] 文案生成请求 | platform=4sapi | model=%s | url=%s", p.model_id, url)
            logger.info("[LLM] 参数设置 | max_tokens=%s | temperature=%s | top_p=%s", 
                       p.max_tokens, p.temperature, p.top_p)
            # 打印完整 payload
            ...
            async with session.post(url, json=payload, headers=headers) as response:
                ...
                if response.status >= 400:
                    ...
                    try:
                        ...
                        logger.error("[LLM] 4sapi 文本生成失败 | status=%s | body=%s", response.status, error_body[:1000])
                    except Exception:
                        pass
                ...
                # 详细输出日志
                logger.info("[LLM] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                logger.info("[LLM] 文案生成响应 | platform=4sapi | model=%s | elapsed=%.2fs", p.model_id, elapsed)
                # 打印完整响应
                ...
                logger.info("[LLM] 文案预览: %s", generated_text[:200] if generated_text else "")
                logger.info("[LLM] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n")
                return GenerationResult(success=True, text=generated_text, raw_response=result)

        except aiohttp.ClientError as e:
            logger.error(f"[4sapi] HTTP request failed [{self.classify_http_error(e)}]: {e}")
    ...
    async def _generate_image_gpt(
        ...
        try:
            ...
            if reference_images:
                ...
                logger.info(f"[4sapi] 使用 GPT 参考图编辑接口 | model={model_id} | reference_count={len(reference_images)}")
                ...
                for idx, base64_str in enumerate(reference_images):
                    try:
                        ...
                        logger.info(f"[4sapi] 添加参考图 {idx} | format={img_format} | size={len(img_data)} bytes")
                    except Exception as e:
                ...
                payload_summary = {
                    "model": model_id,
                    "prompt": formatted_prompt[:200] + ("..." if len(formatted_prompt) > 200 else ""),
                    "n": p.count,
                    "quality": "high",
                ...
                logger.info("[Image] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                logger.info("[Image] GPT参考图编辑请求 | platform=4sapi | model=%s | url=%s | count=%s", model_id, url, p.count)
                logger.info("[Image] 请求 payload: %s", json.dumps(payload_summary, ensure_ascii=False, indent=2))
                ...
                async with session.post(url, data=form_data, headers=headers) as response:
                    ...
                    elapsed = time.time() - start_time
                    logger.info("[Image] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                    logger.info("[Image] GPT参考图编辑响应 | platform=4sapi | elapsed=%.2fs | model=%s", elapsed, model_id)
                    ...
                    logger.info("[Image] 生成图片数量: urls=%d | base64=%d", 
                               len(image_urls), len(image_base64_list))
                    if image_urls:
                ...
                logger.info("[Image] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                logger.info("[Image] 图片生成请求 | platform=4sapi | model=%s | url=%s | count=%s", model_id, url, p.count)
                ...
                logger.info("[Image] 图片尺寸设置 | original=%s | normalized=%s", p.ratio, normalized_size)
                ...
                async with session.post(url, json=payload, headers=headers) as response:
                    ...
                    elapsed = time.time() - start_time
                    logger.info("[Image] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                    logger.info("[Image] 图片生成响应 | platform=4sapi | elapsed=%.2fs | model=%s", elapsed, model_id)
                    logger.info("[Image] 响应内容: %s", json.dumps(result, ensure_ascii=False, indent=2)[:2000])
    ...
    async def _generate_image_gemini(
        ...
        try:
            ...
            if reference_images:
                # 使用数组格式：包含文本和参考图
                logger.info(f"[GeminiImage] 使用参考图编辑 | count={len(reference_images)}")
                content = [
                    {
                        "type": "text",
                ...
                for idx, base64_str in enumerate(reference_images):
                    try:
                        ...
                        logger.info(f"[GeminiImage] 添加参考图 {idx} | data_url_len={len(data_url)}")
                    except Exception as e:
            ...
            logger.info("[GeminiImage] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            logger.info("[GeminiImage] Gemini 图片生成 | model=%s | url=%s", model_id, url)
            logger.info("[GeminiImage] 提示词: %s", formatted_prompt[:200])
            logger.info("[GeminiImage] 参考图数量: %d", len(reference_images) if reference_images else 0)
            ...
            async with session.post(url, json=payload, headers=headers) as response:
                ...
                elapsed = time.time() - start_time
                logger.info("[GeminiImage] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                logger.info("[GeminiImage] Gemini 图片生成响应 | elapsed=%.2fs", elapsed)
                ...
                if isinstance(content, str):
                    ...
                    if match:
                        ...
                        logger.info("[GeminiImage] 提取到 Base64 图片（markdown格式）| length=%d", len(base64_str))
                        image_base64_list.append(base64_str)
                    else:
                        # 尝试直接匹配 data URL（无 markdown 包装）
                        if content.startswith('data:image/'):
                            logger.info("[GeminiImage] 提取到 Base64 图片（直接data URL）| length=%d", len(content))
                            image_base64_list.append(content)
                        else:
                            logger.warning("[GeminiImage] content 字符串中未找到图片数据 | content_len=%d | content_preview=%s", 
                                          len(content), content[:200])
                
                # 格式2：content 是数组，包含多个内容块（文本 + 图片）
                elif isinstance(content, list):
                    logger.info("[GeminiImage] content 是数组格式 | len=%d", len(content))
                    for item in content:
                        if isinstance(item, dict):
                            ...
                            if item_type == "image_url":
                                ...
                                if url.startswith('data:image/'):
                                    logger.info("[GeminiImage] 提取到 Base64 图片（数组格式）| length=%d", len(url))
                                    image_base64_list.append(url)
                            elif item_type == "text":
                                # 文本内容，跳过
                                logger.debug("[GeminiImage] 跳过文本块 | text=%s", item.get("text", "")[:100])
                
                # 格式3：content 为空，检查 message 的其他字段
                else:
                    logger.warning("[GeminiImage] content 类型未知 | type=%s | content=%s", 
                                  type(content).__name__, str(content)[:200])
                
                logger.info("[GeminiImage] 图片提取结果 | base64_count=%d", len(image_base64_list))
                logger.info("[GeminiImage] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n")
