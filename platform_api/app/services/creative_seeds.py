...
class CreativeSeedBank:
    ...
    EMOTION_SEEDS: List[EmotionSeed] = [
        ...
        EmotionSeed(
            ...
            example_phrases=["犹豫了很久", "纠结了好久", "看了好多评价", "对比了好几款"],
    ...
    @classmethod
    def get_multiple_seeds(cls, count: int = 3, exclude_ids: Optional[List[str]] = None) -> List[CreativeSeed]:
        """
        获取多个不同的创意种子组合
    ...
    def get_seed_by_id(cls, seed_id: str) -> Optional[CreativeSeed]:
        ...
        try:
            ...
            # 匹配开头
            opening = next((o for o in cls.OPENING_SEEDS if o.type.value[:2] == opening_prefix), None)
            emotion = next((e for e in cls.EMOTION_SEEDS if e.type.value[:2] == emotion_prefix), None)
            ending = next((en for en in cls.ENDING_SEEDS if en.type.value[:2] == ending_prefix), None)

            if opening and emotion and ending:
                return CreativeSeed(opening=opening, emotion=emotion, ending=ending, seed_id=seed_id)
        except Exception:
            pass
    ...
    def get_different_seed(cls, current_seed_id: str) -> CreativeSeed:
        ...
        if current_seed:
            ...
            # 选择不同的开头
            available_openings = [o for o in cls.OPENING_SEEDS if o.type not in exclude_openings]
            if not available_openings:
                available_openings = cls.OPENING_SEEDS

            # 选择不同的情感
            available_emotions = [e for e in cls.EMOTION_SEEDS if e.type not in exclude_emotions]
            if not available_emotions:
                available_emotions = cls.EMOTION_SEEDS

            # 选择不同的结尾
            available_endings = [en for en in cls.ENDING_SEEDS if en.type not in exclude_endings]
...
# 便捷函数
def get_creative_seeds_for_generation(count: int = 3, used_seed_ids: Optional[List[str]] = None) -> List[CreativeSeed]:
    """
    为文案生成获取创意种子
...
def get_next_creative_seed_for_retry(current_seed_id: str, used_seed_ids: Optional[List[str]] = None) -> CreativeSeed:
    """
    为重试获取新的创意种子
