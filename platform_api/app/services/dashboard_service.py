...
class DashboardService:
    ...
    async def get_stats(
        ...
        """
        logger.debug(
            "[DashboardService] get_stats | owner=%s | start_date=%s | end_date=%s | filter_operator_id=%s",
            owner_admin_id,
            start_date,
            end_date,
            filter_operator_id,
        ...
        effective_operator_id = (
            filter_operator_id if filter_operator_id is not None else owner_admin_id
        )
        ...
        sub_user_filter = (
            ...
            else [SubUser.owner_operator_id == effective_operator_id]
        )
        gen_item_filter = (
            []
            if effective_operator_id is None
            else [GenerationItem.owner_operator_id == effective_operator_id]
        )
        ...
        gen_query = (
            ...
            if gen_item_filter
            else select(func.count(GenerationItem.id)).where(
                and_(
                    GenerationItem.created_at >= range_start,
                    GenerationItem.created_at < range_end,
                )
    ...
    async def get_sub_user_stats(
        ...
        """
        logger.debug(
            "[DashboardService] get_sub_user_stats | sub_user_id=%s | start_date=%s | end_date=%s",
            sub_user_id,
            start_date,
            end_date,
        )
    ...
    async def get_trend_data(
        db: AsyncSession, owner_admin_id: Optional[int], days: int = 7
    ) -> List[Dict[str, Any]]:
        """
        获取趋势数据（最近N天）
        ...
        """
        logger.debug(
            "[DashboardService] get_trend_data | owner=%s | days=%s",
            owner_admin_id,
            days,
        )
        ...
        for i in range(days - 1, -1, -1):
            ...
            base_filter = (
                ...
                else [GenerationItem.owner_operator_id == owner_admin_id]
            )
    ...
    async def get_recent_tasks(
        ...
        """
        logger.debug(
            "[DashboardService] get_recent_tasks | owner=%s | limit=%s | page=%s | filter_operator=%s | start=%s | end=%s",
            owner_admin_id,
            limit,
            page,
            filter_operator_id,
        ...
        if filter_operator_id is not None:
            base_query = base_query.where(
                GenerationTask.owner_operator_id == filter_operator_id
            )
        elif owner_admin_id is not None:
            base_query = base_query.where(
                GenerationTask.owner_operator_id == owner_admin_id
            )
        ...
        offset = (page - 1) * limit
        paginated_query = (
            base_query.order_by(desc(GenerationTask.created_at))
            .offset(offset)
            .limit(limit)
        )
        ...
        if tasks:
            ...
            count_stmt = (
                select(
                    ...
                    func.sum(
                        func.cast(GenerationItem.status == "queued", Integer)
                    ).label("queued"),
                    func.sum(
                    ...
                    func.sum(
                        func.cast(GenerationItem.status == "failed", Integer)
                    ).label("failed"),
                    func.sum(
                        func.cast(GenerationItem.status == "paused", Integer)
                    ).label("paused"),
                    func.sum(
                        func.cast(
                            GenerationItem.distribution_status == "pending_publish",
                            Integer,
                        )
    ...
    async def get_failed_tasks(
        db: AsyncSession, owner_admin_id: Optional[int], limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        获取有失败子任务的任务列表
        ...
        """
        logger.debug(
            "[DashboardService] get_failed_tasks | owner=%s | limit=%s",
            owner_admin_id,
            limit,
        )
        ...
        query = (
            ...
            .order_by(desc(GenerationTask.updated_at))
            .limit(limit)
        )
        ...
        for task in tasks:
            ...
            error_msg = (
                ...
                if failed_item and failed_item.error_message
                else "未知错误"
            )
        ...
        if dismissed_task_ids:
            failed_tasks = [
                t for t in failed_tasks if t["id"] not in dismissed_task_ids
            ]

        return failed_tasks

    @staticmethod
    async def dismiss_alert(db: AsyncSession, task_id: int) -> bool:
        """
        清除单条告警
    ...
    async def dismiss_all_alerts(
        db: AsyncSession, owner_admin_id: Optional[int] = None
    ) -> int:
        """
        清除所有告警
        ...
        """
        logger.debug(
            "[DashboardService] dismiss_all_alerts | owner_admin_id=%s", owner_admin_id
        )
    ...
    async def get_operator_list(db: AsyncSession) -> List[Dict[str, Any]]:
        ...
        query = (
            select(Operator).where(Operator.status != "disabled").order_by(Operator.id)
        )
        result = await db.execute(query)
        operators = result.scalars().all()

        return [{"id": op.id, "name": op.nickname} for op in operators]
