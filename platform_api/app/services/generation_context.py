...
class GenerationInputData:
    ...
    template_seeds: List[Dict[str, Any]] = field(
        default_factory=list
    )  # 创意种子（实际为 Dict）
    # 种子候选池：记录哪些种子类型原本是 "auto" 以及可选的全部候选项
    # 用于去重重试时重新随机选取不同的种子，产生不同的提示词
    seed_candidates_for_auto: Optional[Dict[str, List[Dict[str, str]]]] = (
        None  # {seed_type: [{name, template}, ...]}
    )
    ...
    image_dedup_scope: List[str] = field(
        default_factory=lambda: ["subuser_image_history"]
    )
