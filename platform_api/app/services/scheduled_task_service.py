...
class ScheduledTaskService:
    ...
    async def update_scheduled_task(
        ...
        if 'schedule_type' in update_data or 'schedule_config_json' in update_data:
            ...
            schedule_config = update_data.get('schedule_config_json', task.schedule_config_json)
    ...
    async def delete_scheduled_task(
        ...
        logger.info(
            f"[ScheduledTaskService] 删除定时任务成功 | id={task_id}"
        )
    ...
    async def execute_scheduled_task(
        ...
        try:
            ...
            if task.task_type == "benchmark" and task.benchmark_material_ids_json and len(task.benchmark_material_ids_json) > 0:
                selected_benchmark_material_id = random.choice(task.benchmark_material_ids_json)
            ...
            sub_user_ids = task.sub_user_ids_json if isinstance(task.sub_user_ids_json, list) else None
            ...
            generation_task = await GenerationService.create_generation_task(
                ...
                benchmark_material_id=selected_benchmark_material_id if task.task_type == "benchmark" else None,
            ...
            import json
            now = datetime.now()
            sub_user_count = len(task.sub_user_ids_json) if isinstance(task.sub_user_ids_json, list) else 0
    ...
    def _calculate_next_execution_at(
        ...
        if schedule_type == "daily":
            ...
            end_date = dt.strptime(end_date_str, "%Y-%m-%d").replace(hour=23, minute=59, second=59)
            ...
            for time_str in times:
                hour, minute = map(int, time_str.split(":"))
                next_time = current_date.replace(hour=hour, minute=minute, second=0, microsecond=0)
    ...
    async def enrich_task_response(
        ...
        if task.benchmark_material_ids_json:
            query = select(Material.title).where(Material.id.in_(task.benchmark_material_ids_json))
            result = await db.execute(query)
        ...
        if task.sub_user_ids_json:
            query = select(SubUser.nickname).where(SubUser.id.in_(task.sub_user_ids_json))
            result = await db.execute(query)
        ...
        if task.owner_operator_id:
            query = select(Operator.nickname).where(Operator.id == task.owner_operator_id)
            result = await db.execute(query)
