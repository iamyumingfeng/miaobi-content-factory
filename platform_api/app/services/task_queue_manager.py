...
class TaskQueueManager:
    ...
    def _acquire_lock(self, timeout: int = LOCK_TIMEOUT) -> Optional[redis.lock.Lock]:
        ...
        try:
            ...
            if lock.acquire():
                ...
                logger.debug(f"调度锁被占用，跳过本次调度 | lock_key={DISPATCH_LOCK_KEY}")
                return None
    ...
    def enqueue_task(self, item_id: int, owner_operator_id: int, priority: int = 0) -> int:
        """
        将任务加入等待队列
    ...
    def add_active_task(self, item_id: int, celery_task_id: str) -> None:
        ...
        try:
            ...
            logger.info(f"添加活跃任务: item_id={item_id}, celery_task_id={celery_task_id}")
        except RedisError as e:
    ...
    def dispatch_from_queue(self, owner_operator_id: int, max_dispatch: int = 10) -> int:
        """
        ...
        for _ in range(max_dispatch):
            ...
            try:
                # 检查是否可以调度
                if not self.can_dispatch():
                    logger.info(f"⏸ 达到最大并发数，暂停调度 | active={self.get_active_count()} | max={self.get_max_concurrent()}")
                    break
                
                # 从队列取出任务
                task_data = self.dequeue_task()
                if not task_data:
                    logger.info(f"✔ 等待队列已空，调度完成 | dispatched={dispatched_count}")
                    break
                ...
                try:
                    ...
                    dispatched_count += 1
                    logger.info(f"▶ 调度任务到Worker | item_id={item_id} | celery_id={celery_task.id}")
