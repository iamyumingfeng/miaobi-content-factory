...
async def list_scheduled_tasks(
    ...
    status: Optional[str] = Query(None, description="状态筛选：active / paused / disabled"),
    task_type: Optional[str] = Query(None, description="任务类型筛选：custom / benchmark"),
    keyword: Optional[str] = Query(None, description="关键词搜索"),
    is_active: Optional[bool] = Query(None, description="是否启用筛选"),
    operator_id: Optional[int] = Query(None, description="创作管理员ID（仅超级管理员使用）"),
...
async def update_scheduled_task(
    ...
    logger.info(
        f"[API] 开始更新定时任务 | id={task_id} | owner={owner_operator_id}"
    )
    
    # 获取旧任务数据（用于记录操作日志）
    old_task = await ScheduledTaskService.get_scheduled_task(db, task_id, owner_operator_id)
    if not old_task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="定时任务不存在或无权访问"
...
async def execute_scheduled_task_now(
    ...
    logger.info(
        f"[API] 立即执行定时任务 | id={task_id} | owner={owner_operator_id}"
    )
    ...
    # 执行任务
    success, generation_task_id, error_message = await ScheduledTaskService.execute_scheduled_task(
        db, task_id
    )
...
@router.get("/{task_id}/executions", response_model=ApiResponse[PaginatedResponse[ScheduledTaskExecutionLog]])
