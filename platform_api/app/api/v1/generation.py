...
from app.schemas.generation import (BatchPauseRequest, BatchRetryRequest,
                                    DebugGenerateImagesRequest,
                                    DebugGenerateImagesResponse,
                                    DebugGenerateTextRequest,
                                    DebugGenerateTextResponse,
                                    DebugRerunRequest, ExecutionLogResponse,
                                    GenerationItemDetailResponse,
                                    GenerationItemResponse,
                                    GenerationTaskCreate,
                                    GenerationTaskProgressLogResponse,
                                    GenerationTaskResponse,
                                    GenerationTaskUpdate, MaterialBrief,
                                    SubUserItemsListResponse, TemplateInfo)
from app.services.generation_service import (GenerationService, _process_url,
                                             _process_urls)
...
# ============================================
@router.get(
    "/tasks", response_model=ApiResponse[PaginatedResponse[GenerationTaskResponse]]
)
    ...
    for task in items:
        ...
        response_items.append(
            GenerationTaskResponse(
                ...
                distributed_count=_lc.get(
                    "distributed_count", getattr(task, "distributed_count", 0) or 0
                ),
                pending_publish_count=_lc.get(
...
async def create_generation_task(
    ...
    logger.info(
        "[API] 开始创建生成任务 | owner=%s | name=%s | templates=%s | sub_users=%s",
        owner_operator_id,
        request.name,
        request.template_ids,
        request.sub_user_ids,
    )
    logger.info(
        "[API] 请求数据: %s", json.dumps(request.model_dump(), ensure_ascii=False)
    )
    ...
    try:
        task = await GenerationService.create_generation_task(
            ...
            dedup_enabled=(
                request.dedup_enabled if request.dedup_enabled is not None else False
            ),
            dedup_threshold=request.dedup_threshold,
            dedup_retry_count=request.dedup_retry_count,
            dedup_scope=request.dedup_scope,
            image_dedup_enabled=(
                request.image_dedup_enabled
                if request.image_dedup_enabled is not None
                else False
            ),
    ...
    logger.info(
        "[API] 任务创建成功 | task_id=%s | owner=%s | total_items=%s | queued=%s",
        task.id,
        owner_operator_id,
        task.total_count,
        task.queued_count,
...
async def get_generation_task(
    ...
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="生成任务不存在"
        )
    ...
    if task.task_templates:
        ...
        if sorted_templates and sorted_templates[0].template:
            ...
            from app.models import (CreativeSeed, TemplateAttachment,
                                    TemplatePlatform)
            from app.models import ViralType as ViralTypeModel

            attachments_result = await db.execute(
                select(TemplateAttachment)
                .where(TemplateAttachment.template_id == first_template.id)
            ...
            if first_template.platform_id:
                platform_result = await db.execute(
                    select(TemplatePlatform.name).where(
                        TemplatePlatform.id == first_template.platform_id
                    )
                )
            ...
            if viral_type_value:
                if viral_type_value == "auto":
                    ...
                    vt_result = await db.execute(
                        select(ViralTypeModel.label).where(
                            ViralTypeModel.value == viral_type_value
                        )
                    )
...
@router.post(
    "/tasks/{id}/recalculate", response_model=ApiResponse[GenerationTaskResponse]
)
async def recalculate_task_counts(
    ...
    logger.info(
        "[API] recalculate_task_counts | task_id=%s | owner=%s", id, owner_operator_id
    )
...
@router.get(
    "/tasks/{id}/progress-logs",
    response_model=ApiResponse[list[GenerationTaskProgressLogResponse]],
)
...
@router.get(
    "/tasks/{id}/items",
    response_model=ApiResponse[PaginatedResponse[GenerationItemResponse]],
)
async def list_generation_items(
    ...
    logger.debug(
        "[API] list_generation_items | task_id=%s | owner=%s | page=%s | limit=%s | "
    ...
    logger.debug(
        "[API] 子任务列表 | task_id=%s | total=%s | returned=%s | status_breakdown=%s",
        id,
        total,
        len(items),
        status_counts,
    ...
    for item in items:
        if item.sub_user:
            item.sub_user_name = (
                item.sub_user.nickname
                or item.sub_user.display_name
                or str(item.sub_user_id)
            )
    ...
    for item in items:
        if item.generated_image_urls_json:
            item.generated_image_urls_json = _process_urls(
                item.generated_image_urls_json, prefer_url
            )
        if item.generated_image_thumbnails_json:
            item.generated_image_thumbnails_json = _process_urls(
                item.generated_image_thumbnails_json, prefer_url
            )
        if item.generated_video_url:
            item.generated_video_url = _process_url(
                item.generated_video_url, prefer_url
            )
...
async def get_generation_item(
    ...
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="生成子任务不存在"
        )
...
@router.get(
    "/items/{id}/detail", response_model=ApiResponse[GenerationItemDetailResponse]
)
async def get_generation_item_detail(
    ...
    from app.models import (GenerationTask,
                            MaterialAttachment, TemplateAttachment)
    ...
    if item.sub_user_id:
        ...
        if sub_user:
            sub_user_name = (
                sub_user.nickname or sub_user.display_name or str(sub_user.id)
            )
    ...
    response_data = GenerationItemDetailResponse(
        ...
        text_file_url=(
            _process_url(item.text_file_url, prefer_url) if item.text_file_url else None
        ),
        ...
        input_template_images_json=(
            _process_urls(template_images, prefer_url) if template_images else None
        ),
        ...
        input_product_selling_points=(
            template_obj.product_selling_points if template_obj else None
        ),
        ...
        input_benchmark_title=(
            benchmark_material.title
            if benchmark_material
            else item.input_benchmark_title
        ),
        input_benchmark_content=(
            benchmark_material.content
            if benchmark_material
            else item.input_benchmark_content
        ),
        input_benchmark_topic=(
            benchmark_material.topic
            if benchmark_material
            else item.input_benchmark_topic
        ),
        input_benchmark_images_json=(
            _process_urls(benchmark_images, prefer_url) if benchmark_images else None
        ),
...
async def retry_generation_item(
    ...
    logger.debug(
        "[API] retry_generation_item | item_id=%s | owner=%s", id, owner_operator_id
    )
    try:
        ...
        logger.info(
            "[API] 子任务重试成功 | item_id=%s | owner=%s | new_status=%s",
            id,
            owner_operator_id,
            item.status,
        )
...
@router.post(
    "/items/{id}/regenerate", response_model=ApiResponse[GenerationItemResponse]
)
async def regenerate_generation_item(
    ...
    logger.debug(
        "[API] regenerate_generation_item | item_id=%s | owner=%s",
        id,
        owner_operator_id,
    )
    try:
        ...
        logger.info(
            "[API] 子任务重新生成成功 | item_id=%s | owner=%s | new_status=%s",
            id,
            owner_operator_id,
            item.status,
        )
...
@router.post(
    "/items/batch-retry", response_model=ApiResponse[list[GenerationItemResponse]]
)
async def batch_retry_generation_items(
    ...
    logger.debug(
        "[API] batch_retry | item_ids=%s | task_id=%s | owner=%s",
        request.item_ids,
        request.task_id,
        owner_operator_id,
    )
    try:
        ...
        logger.info(
            "[API] 批量重试成功 | owner=%s | count=%s", owner_operator_id, len(items)
        )
...
async def pause_generation_item(
    ...
    logger.debug(
        "[API] pause_generation_item | item_id=%s | owner=%s", id, owner_operator_id
    )
    try:
        ...
        logger.info(
            "[API] 子任务已暂停 | item_id=%s | owner=%s | new_status=%s",
            id,
            owner_operator_id,
            item.status,
        )
...
async def resume_generation_item(
    ...
    logger.debug(
        "[API] resume_generation_item | item_id=%s | owner=%s", id, owner_operator_id
    )
    try:
        item = await GenerationService.toggle_pause_item(
        ...
        logger.info(
            "[API] 子任务已继续 | item_id=%s | owner=%s | new_status=%s",
            id,
            owner_operator_id,
            item.status,
        )
...
@router.post(
    "/items/batch-pause", response_model=ApiResponse[list[GenerationItemResponse]]
)
async def batch_pause_generation_items(
    ...
    logger.debug(
        "[API] batch_pause | action=%s | item_ids=%s | owner=%s",
        action,
        request.item_ids,
        owner_operator_id,
    )
    ...
    try:
        ...
        return success_response(
            data=items, message=f"成功{action} {len(items)} 个子任务"
        )
    except Exception as e:
        logger.warning(
            "[API] 批量%s失败 | owner=%s | error=%s", action, owner_operator_id, e
        )
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
# ============================================
@router.get(
    "/items/{id}/execution-logs", response_model=ApiResponse[list[ExecutionLogResponse]]
)
async def get_item_execution_logs(
    ...
    logger.debug(
        "[API] get_item_execution_logs | item_id=%s | owner=%s", id, owner_operator_id
    )
    try:
            ...
            response_logs.append(
                {
                    ...
                    "node_status": (
                        log.node_status.value
                        if hasattr(log.node_status, "value")
                        else log.node_status
                    ),
        ...
        logger.debug(
            "[API] 执行日志列表 | item_id=%s | count=%s | nodes=%s",
            id,
            len(logs),
            node_summary,
        )
...
# ============================================
@router.post(
    "/items/{id}/debug-rerun", response_model=ApiResponse[GenerationItemResponse]
)
async def debug_rerun_item(
    ...
    logger.debug(
        "[API] debug_rerun_item | item_id=%s | owner=%s | prompt_override=%s | platform_override=%s | model_override=%s",
        id,
        owner_operator_id,
        (request.prompt_override[:50] + "...") if request.prompt_override else None,
        request.model_platform_override,
        request.model_id_override,
    )
    ...
    try:
        ...
        logger.info(
            "[API] debug_rerun_item: 重置子任务状态 | item_id=%s | old_status=%s",
            id,
            item.status,
        )
        ...
        if request.prompt_override:
            ...
            logger.debug(
                "[API] debug_rerun_item: 覆盖提示词 | item_id=%s | len=%s",
                id,
                len(request.prompt_override),
            )
        if request.model_platform_override:
            item.model_platform = request.model_platform_override
            logger.debug(
                "[API] debug_rerun_item: 覆盖平台 | item_id=%s | platform=%s",
                id,
                request.model_platform_override,
            )
        if request.model_id_override:
            ...
            logger.debug(
                "[API] debug_rerun_item: 覆盖模型 | item_id=%s | model_id=%s",
                id,
                request.model_id_override,
            )
        ...
        # 异步执行重跑
        logger.info(
            "[API] debug_rerun_item: 分发 Celery 任务 | item_id=%s | owner=%s",
            id,
            owner_operator_id,
        )
...
@router.post(
    "/debug/generate-text", response_model=ApiResponse[DebugGenerateTextResponse]
)
async def debug_generate_text(
    ...
    logger.info(
        "[API] debug_generate_text | item_id=%s | owner=%s",
        request.item_id,
        owner_operator_id,
    )
    ...
    try:
        # 获取子任务
        item = await GenerationService.get_generation_item(
            db, request.item_id, owner_operator_id
        )
        if not item:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="生成子任务不存在"
        ...
        # 获取系统提示词和用户提示词
        system_prompt = (
            request.system_prompt_override or item.output_system_text_prompt or ""
        )
        ...
        if not model_platform or not model_id:
            ...
            if not result:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="没有可用的文本模型"
                )
        ...
        formatted_system = (
            adapter.format_prompt(system_prompt, variable_values)
            if system_prompt
            else None
        )
...
@router.post(
    "/debug/generate-images", response_model=ApiResponse[DebugGenerateImagesResponse]
)
async def debug_generate_images(
    ...
    logger.info(
        "[API] debug_generate_images | item_id=%s | owner=%s",
        request.item_id,
        owner_operator_id,
    )
    ...
    try:
        ...
        model_platform = request.model_platform_override or item.model_platform
        model_id = request.model_id_override or item.model_id
        platform_lower = model_platform.lower() if model_platform else ""

        # 获取用户提示词（生图模型不使用系统提示词，每条 image_prompt 自包含完整创作指导）
        user_prompts = (
            request.user_prompts_override or item.aigc_user_image_prompts_json or []
        )

        # 确定生成图片数量
        # 注意：如果 request.image_count_override 是 0，应该使用 len(user_prompts) 或 1
        if (
            request.image_count_override is not None
            and request.image_count_override > 0
        ):
        ...
        logger.info(
            "[API] debug_generate_images | user_prompts count=%s | image_count=%s",
            len(user_prompts),
            image_count,
        )
        ...
        if not model_platform or not model_id:
            ...
            if not result:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="没有可用的图片模型"
                )
            config, platform = result
            model_platform = platform
            model_id = config.model_id
        ...
        logger.info(
            "[API] debug_generate_images | 模型适配器已创建 | platform=%s | model_id=%s | api_key=%s | extra_params_keys=%s",
            platform,
            config.model_id,
        ...
        if platform == "kling":
            logger.info(
                "[API] debug_generate_images | [Kling诊断] access_key_id=%s | access_key_secret=%s",
                "present" if config.extra_params.get("access_key_id") else "MISSING",
                (
                    "present"
                    if config.extra_params.get("access_key_secret")
                    else "MISSING"
                ),
            )
        ...
        if request.reference_image_urls_override is not None:
            ...
            logger.info(
                "[API] debug_generate_images | 使用前端传入的参考图片 | count=%s",
                len(override_urls),
            )
            ...
            if n_product_config == 0 and item.template_id:
                ...
                att_result = await db.execute(
                    select(TemplateAttachment).where(
                        TemplateAttachment.template_id == item.template_id
                    )
                )
            ...
            logger.info(
                "[API] debug_generate_images | 配置数量 | product=%s | benchmark=%s",
                n_product_config,
                n_benchmark_config,
            )
            ...
            logger.info(
                "[API] debug_generate_images | 前端图片分割 | product=%s | benchmark=%s",
                len(product_urls),
                len(benchmark_urls),
            )
        else:
            ...
            if item.input_benchmark_image_enabled and item.input_benchmark_images_json:
                ...
                logger.info(
                    "[API] debug_generate_images | 对标图 | count=%s",
                    len(benchmark_urls),
                )
            ...
            if item.template_id:
                ...
                template_query = (
                    select(Template)
                    .options(selectinload(Template.attachments))
                ...
                if template and template.attachments:
                    ...
                    logger.info(
                        "[API] debug_generate_images | 模板产品图已加载 | count=%s",
                        len(product_urls),
                    )
        ...
        add_watermark = (
            bool(item.input_watermark) if item.input_watermark is not None else True
        )
        ...
        if item.input_benchmark_image_roles_json:
            ...
            if isinstance(roles_data, dict):
                ...
                for i in range(len(benchmark_urls)):
                    ...
                    role_tags = (
                        roles_data.get(key1)
                        or roles_data.get(key2)
                        or roles_data.get(key3, [])
                    )
                ...
                logger.info(
                    "[API] debug_generate_images | 对标图角色配置 | count=%s | roles=%s",
                    len(bench_roles_list),
                    [r.get("description", "") for r in bench_roles_list],
                )
        ...
        logger.info(
            "[API] debug_generate_images | 参考图转换完成 | benchmark=%s | product=%s",
            len(benchmark_b64_list),
            len(product_b64_list),
        )

        logger.info(
            "[API] debug_generate_images | 图片尺寸配置 | size_ratio=%s | image_count=%s | benchmark=%s | product=%s",
            image_size_ratio,
            image_count,
            len(benchmark_b64_list),
            len(product_b64_list),
        ...
        logger.info(
            "[API] debug_generate_images | adapter_kwargs=%s",
            {
                k: v
        ...
        for idx, user_prompt in enumerate(user_prompts[:image_count]):
            ...
            logger.info(
                "[API] debug_generate_images | 生成图片 %d/%d | 对标图索引=%s | 产品图=%s张",
                idx + 1,
            ...
            logger.info(
                "[API] debug_generate_images | 生成图片 %d/%d | has_refs=%s",
                idx + 1,
                len(user_prompts[:image_count]),
                bool(selected_refs),
            )
            ...
            try:
                ...
                logger.error(
                    "[API] debug_generate_images | 图片生成异常 | idx=%s | error=%s | error_type=%s",
                    idx,
                    str(gen_error),
            ...
            if result.success and result.image_urls:
                ...
                logger.warning(
                    "[API] debug_generate_images | 图片生成失败 | idx=%s | error=%s",
                    idx,
        ...
        response = DebugGenerateImagesResponse(
            ...
            reference_image_urls=(
                benchmark_urls + product_urls
                if (benchmark_urls or product_urls)
                else None
            ),
        )

        logger.info(
            "[API] debug_generate_images 成功 | item_id=%s | count=%s",
            request.item_id,
            len(image_urls),
        )
        ...
        logger.error(
            "[API] debug_generate_images 失败 | error=%s | error_type=%s\n%s",
            str(e),
            type(e).__name__,
...
async def get_sub_user_items(
    page: int = Query(1, ge=1, description="页码"),
    limit: int = Query(20, ge=1, le=100, description="每页数量"),
    distribution_status: Optional[str] = Query(
        None, description="分发状态筛选（pending_publish/published）"
    ),
    start_date: Optional[str] = Query(None, description="开始日期（YYYY-MM-DD）"),
    end_date: Optional[str] = Query(None, description="结束日期（YYYY-MM-DD）"),
    payload: dict = Depends(get_token_payload_required),
    ...
    if user_type != "sub_user":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="只有创作者可以访问此接口"
        )
    ...
    logger.debug(
        "[API] get_sub_user_items | sub_user=%s | page=%s | limit=%s | distribution_status=%s",
        sub_user_id,
        page,
        limit,
        distribution_status,
    ...
    try:
        ...
        logger.info(
            "[API] 获取创作者内容列表成功 | sub_user=%s | count=%s",
            sub_user_id,
            len(result["items"]),
        )
        ...
        logger.warning(
            "[API] 获取创作者内容列表失败 | sub_user=%s | error=%s", sub_user_id, e
        )
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def publish_generation_item(
    ...
    logger.debug(
        "[API] publish_generation_item | item_id=%s | owner=%s | sub_user=%s",
        id,
        owner_operator_id,
        sub_user_id,
    )
    ...
    try:
        item = await GenerationService.publish_item(
            db, item_id=id, owner_operator_id=owner_operator_id, sub_user_id=sub_user_id
        )
        logger.info(
            "[API] 子任务已标记为已发布 | item_id=%s | owner=%s", id, owner_operator_id
        )
