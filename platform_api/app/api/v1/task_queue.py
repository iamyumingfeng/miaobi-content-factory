...
from app.utils.deps import get_token_payload_required, get_super_admin_required
...
async def get_worker_status(
    ...
    try:
        ...
        logger.info(f"查询 Worker 状态 | running={worker_running_count} | idle={worker_idle_count} | total={worker_total}")
...
async def get_queue_status(
    ...
    try:
        ...
        logger.info(f"查询队列状态 | active={status_data['active_count']} | waiting={status_data['waiting_count']}")
...
async def update_queue_config(
    ...
    try:
        ...
        logger.info(f"更新队列配置 | max_concurrent={max_concurrent} | operator={payload.get('sub')}")
        return success_response(data={
...
async def trigger_dispatch(
    ...
    try:
        ...
        logger.info(f"手动触发调度 | dispatched={dispatched_count} | operator={owner_operator_id}")
        return success_response(data={
...
async def clear_waiting_queue(
    ...
    try:
        ...
        logger.warning(f"清空等待队列 | count={waiting_count} | operator={payload.get('sub')}")
        return success_response(data={
...
async def recover_stale_tasks(
    ...
    try:
        ...
        logger.info(f"恢复超时任务 | timeout={timeout_minutes}min | stale_count={len(stale_tasks)}")
        return success_response(data={
